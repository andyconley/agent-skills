...
require_text "$JIRA_FILE" "12. Semantic link findings, as a findings block."
...
require_text "$JIRA_FILE" "Read status category only. Never use a project status name in a finding, except a ready status the request supplied."
...
require_text "$JIRA_FILE" "A rejected item counts as done. Report its resolution."
...
require_text "$JIRA_FILE" "A forward edge that agrees with both tickets' text is not a finding."
