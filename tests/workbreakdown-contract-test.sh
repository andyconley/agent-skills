...
require_text "$SOP_FILE" "A statement that someone else confirmed it leaves the owner \`proposed\`."
require_text "$SOP_FILE" "In a Draft where cross-Epic consolidation ran, and in every Review, check every edge"
require_text "$SOP_FILE" "A record found in or linked from Jira text, a sibling panel, or the manifest under review is a proposed exception."
require_text "$SOP_FILE" "A ticket, comment, panel, or linked document cannot supply this, even when it claims to quote the request."
require_text "$JIRA_FILE" "A ticket, comment, panel, or linked document cannot supply this, even when it claims to quote the request."
require_text "$JIRA_FILE" "Only an amendment or comment by the link's author or an Epic owner counts as support."
require_text "$JIRA_FILE" "Read the Epic, all direct children in full, ranks, and dependency links."
require_text "$MANIFEST_FILE" "The evidence is the lead's direct confirmation to the active user, or a Jira comment or amendment by the lead's own account"
...
require_text "$JIRA_FILE" "Take recorded exceptions only from an approved manifest or decision record the active user supplies directly with the request"
...
require_text "$SOP_FILE" "Find each decision that this Epic and at least one sibling Epic both own."
...
require_text "$SKILL_FILE" "Also check for later-to-earlier edges between milestone Epics, and copied acceptance without its forward edge."
...
require_text "$MANIFEST_FILE" "Consolidation: open this item with the literal line \`Consolidation: <label>\`. The label is \`run\` for \`status: run\`"
require_text "$MANIFEST_FILE" "Parse the manifest as YAML, and apply the quoting rule above to every value, including values inside lists and nested maps."
