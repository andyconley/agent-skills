...
require_text "$MANIFEST_FILE" "When the live Epic has a panel, the update carries breakdown_conventions."
...
require_text "$SOP_FILE" "In a non-interactive run, use a sibling's answer only when all siblings that record it agree, and otherwise use the portable default and record it as \`default\`."
