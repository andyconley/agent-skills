...
| 12 Empty release notes | Fixed. Root-caused to `5140bab`, which replaced the documented `hidden` key with an invented `effect` key in `presetConfig.types`. Reverted to `hidden`, preserving that commit's intent that documentation commits appear. The next release verifies it. |
