...
| `audit-formatting` | Pass | Fail: finds excess bold but adds a second finding about intentional `**one**` emphasis and non-actionable keep commentary |
...
Raw prompts, outputs, metadata, and tool events remain in ignored `validation/raw/installed-best-effort-smoke/`. Independent review scored Codex 8/8 and Claude 4/8; the Claude formatting case failed for an unnecessary second finding against intentional emphasis. No score here converts the documented failures to passes.
