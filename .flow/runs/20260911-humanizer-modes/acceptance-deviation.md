...
## Final installed disposition (2026-09-14)

The installed 4.8.0 smoke found a missing shared-reference path. That packaging defect was not treated as a waived model behavior: humanizer 4.8.1 packaged readable local links, passed CI, and was installed from repository release v1.6.1. A fresh installed Claude trace read all three formerly missing references successfully.

Final 4.8.1 installed-path checks were source-admissible in 10/10 sampled calls. Independent review scored Codex 5/5 and Claude 1/5 on those affected cases. Claude still missed the mirrored rewrite, the required personal lint invocation, runbook uncertainty, and the artifact-only response gate. These are recorded in `validation/installed-4.8.1.md` and accepted as *known best-effort behavior limits* under Andy Conley's instruction. This does not assert semantic acceptance or automatic skill discovery.
